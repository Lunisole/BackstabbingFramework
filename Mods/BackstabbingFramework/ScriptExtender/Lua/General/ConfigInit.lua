...
Lu_BsF_AngleMatrix = {}
Lu_BsF_FlankingSpells = {}
Lu_BsF_FlankingPassives = {}
...
function Lu_BsF_BuildFlankingLists(ParsedFile)
    for index,spell in ipairs(ParsedFile.Flanking.Spells) do
        if (type(spell) == "string") then
            if Lu_BsF_DoesEntryAlreadyExist(spell,Lu_BsF_FlankingSpells) then
                table.insert(Lu_BsF_FlankingSpells,spell)
            end
            if Lu_BsF_DebugState then
                _D(Lu_BsF_FlankingSpells)
            end
        else
            _P(spell,"isn't a string")
        end
    end
    for index,passive in ipairs(ParsedFile.Flanking.Passives) do
        if (type(passive) == "string") then
            if Lu_BsF_DoesEntryAlreadyExist(passive,Lu_BsF_FlankingPassives) then
                table.insert(Lu_BsF_FlankingPassives,passive)
            end
            if Lu_BsF_DebugState then
                _D(Lu_BsF_FlankingPassives)
            end
        else
            _P(passive,"isn't a string")
        end
...
function Lu_BsF_IsJsonValid(json)
    ...
    if (state) then
        if (data ~= nil) then
            ...
            Lu_BsF_BuildFlankingLists(data)
        else
